...
template <class T>
T abs(T a){if(a<0){return a*(-1);} return a;}

void wav_errors_e::read_header(const char *filename, wav_header_s *header_ptr)
{
...
void wav_errors_e::print_info(const wav_header_s *header_ptr)
{
    printf( "-------------------------\n" );
...
void wav_errors_e::extract_data_int16( const char* filename, std::vector<std::vector<short>>& channels_data )
{
...
void wav_errors_e::check_header( const wav_header_s *header_ptr, size_t file_size_bytes )
{
    ...
    if ( header_ptr->chunkSize != file_size_bytes - 8 ) {
       // printf( "HEADER_FILE_SIZE_ERROR\n" );
        throw HeaderFileSizeErrorException();
    }
         ...
         header_ptr->format[3] != 0x45 )
    {
        ...
        throw HeaderWaveErrorException();
    }
    ...
    if ( header_ptr->subchunk1Size != 16 ) {
       // printf( "HEADER_SUBCHUNK1_ERROR\n" );
        throw HeaderFirstSubchunkErrorException();
    }
    ...
    if ( header_ptr->subchunk2Size != file_size_bytes - 44 )
    {
        ...
        throw HeaderSecondSubchunkErrorException();
    }

    //return HEADER_OK;
}

void wav_errors_e::prefill_header(wav_header_s *header_ptr)
{
    // Go to wav_header.h for details
...
void wav_errors_e::fill_header(wav_header_s *header_ptr, int chan_count, int bits_per_sample, int sample_rate, int samples_count_per_chan)
{
...
void wav_errors_e::make_wav_file(const char* filename, int sample_rate, const std::vector< std::vector<short> > &channels_data)
{
...
void wav_errors_e::null_header(wav_header_s *header_ptr)
{
    memset( header_ptr, 0, sizeof(wav_header_s) );
}

void  wav_errors_e::make_mono(const std::vector<std::vector<short> > &source, std::vector< std::vector<short> > &dest_mono)
{
...
void wav_errors_e::make_reverb(std::vector<std::vector<short> > &sounds, int sample_rate, double delay_seconds, float decay)
{
